...
def test_request_id_timeout_then_retry_does_not_reexecute(srv, monkeypatch):
    # owner 超时返回 504 但 entry 仍保留；同 id 重试必须等待原 worker，绝不双写。
    port, calls = srv
    ...
    assert len(calls) == 1  # 原 entry 完成后回放，绝不重建 owner


def test_claim_inflight_never_reclaims_stale(monkeypatch):
    # inflight 无论等待多久都不能被接管；未知执行状态时宁可持续 pending，
    # 也不能用同一 request_id 重新执行破坏性操作。
    ...
    # 伪造等待超时：entry 仍必须被复用，不能重新获得 owner 权限
    e3, is_owner3 = server._claim("rid-x", "hash-x")
    assert is_owner3 is False and e3 is e1
    assert e1.state == "inflight"
    assert server._REQUEST_ID_CACHE.get("rid-x") is e1
