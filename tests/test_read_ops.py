...
def _slide(title_text, body_text, notes_text, has_title=True):
    ...
    shapes.append(
        _FakeShape(
            2, body_text, name="body", kind=14, ph_type=2, left=36, top=90, width=600, height=396
        )
    )
...
def test_ppt_read_slide_summary_structure():
...
def test_ppt_read_slide_summary_missing_fields_empty():
    # 无标题（has_title=False）→ 无标题占位符，回退也找不到文本 → 空串
