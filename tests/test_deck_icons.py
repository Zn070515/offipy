"""deck.render 图表/资源后处理接线测试（monkeypatch 掉转换与真实后处理）。"""
...
def test_render_wires_chart_then_asset_postprocess(tmp_path, monkeypatch):
    """render 依次调用 charts → assets 后处理（顺序 + 实参断言，不走真 convert/Playwright）。"""
    ...
    assert out == str(pptx)
    assert calls == ["charts", "assets"]
    assert tmp_seen["assets"] == tmp_seen["charts"]  # 两阶段作用于同一临时文件
    assert pptx.read_bytes() == b"fake pptx"  # 最终路径拿到转换产物
    assert not list(tmp_path.glob(".d.*.pptx"))  # 临时文件已清理（mkstemp 命名）


def test_render_charts_error_skips_assets(tmp_path, monkeypatch):
    """charts 后处理抛错时，assets 后处理不该被调用，且临时文件清理。"""
    html = tmp_path / "d.html"
    ...
    assert calls == ["charts"]  # assets 不该被调用
