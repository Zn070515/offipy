...
关闭文档（doc_id 必须显式传入或 follow_active=True）。save=True 先保存（从未保存过则自动落盘用户数据目录，不弹另存为）并返回保存路径；save=False 不保存不弹窗，返回 null。
...
保存文档（doc_id 必须显式传入或 follow_active=True）并返回绝对路径。给 path 则另存到该路径；未给 path 则存回原路径（从未保存过自动落盘用户数据目录，不弹另存为）；overwrite=True 允许覆盖已存在文件。
...
把文档（doc_id 必须显式传入或 follow_active=True）导出为 PDF 到指定路径；overwrite=True 允许覆盖已存在文件。
