...
# POSIX 分支（#89/#91）：/ 须位于 token 边界——协议/版本标签（Remote*/CLI/HTTP、
# Remote*/cli/http、Word*/api/v2）的第一个 / 紧贴 * 标签前缀，列入负向 lookbehind
# 排除（#89 不误伤）；真实 POSIX 绝对路径不要求小写/数字特征，全大写（/ALL/UPPER）
# 也脱（#91 修 #89 段特征前瞻的两极盲区，宁多勿漏）。
_PATH_RE = re.compile(
    r"(?:\bfile://(?:[^/\s]*/)?(?:[A-Za-z]:/)?(?:[^\s:;\"'/]+/)*[^\s:;\"']*"  # file:// URL（#78）
    r"|[A-Za-z]:[\\/](?!\/)(?:[^\\/:]+[\\/])*[^\\/:]*?"  # Windows 路径（#80/#81/#82）
    r"(?=[:\r\n\"']|\s+[一-鿿]|\s+doc_id|\s+(?![^\s:;\"'\\/]+[\\/])|\s*$)"
    r"|(?<![:/.@*\w])/"  # POSIX（#89/#91 token 边界：排除 * 标签前缀）
    r"(?:[^\s:;\"'/]+/)+[^\s:;\"']*"
    r"|\\\\[^\s:;\"']+"  # UNC 共享路径
