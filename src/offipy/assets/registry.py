...
class AssetRegistry:
    ...
    def _validate_provider(self, provider: AssetProvider) -> None:
        ...
        if not kinds:
            raise InvalidArgumentError(
                f"asset provider {provider_id!r} declares no kinds")
        for kind in kinds:
    ...
    def provider(self, provider_id: str) -> AssetProvider:
        try:
            ...
            raise InvalidArgumentError(
                f"unknown asset provider {provider_id!r}") from None
    ...
    def resolve(self, uri_or_request: str | AssetRequest) -> ResolvedAsset:
        if isinstance(uri_or_request, str):
            ...
            raise InvalidArgumentError(
                "resolve expects an asset URI string or AssetRequest")
    ...
    def _validate_resolved(self, provider: AssetProvider, request: AssetRequest,
                           resolved: ResolvedAsset) -> None:
        if not isinstance(resolved, ResolvedAsset):
            raise InvalidArgumentError(
                f"asset provider {provider.provider_id!r} did not return a "
                f"ResolvedAsset")
        if resolved.request != request:
            raise InvalidArgumentError(
                f"asset provider {provider.provider_id!r} resolved a different "
                f"request than asked")
