...
class LicensePolicy:
    ...
    def validate_manifest(self, data: Mapping[str, object], *,
        ...
        if actual_count is not None and count != actual_count:
            raise InvalidArgumentError(
                f"vendored manifest count {count} != actual {actual_count}")
        if data.get("redistributable", True) is False:
            raise InvalidArgumentError(
                "vendored manifest requires redistributable assets")
        if license_id == "CC-BY-4.0":
            attribution = data.get("attribution")
            if not isinstance(attribution, str) or not attribution.strip():
                raise InvalidArgumentError(
                    "vendored CC-BY-4.0 manifest requires attribution")
