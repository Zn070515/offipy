...
OPS: dict[str, dict[str, OpSpec]] = {
    ...
    "diagram": {
        "build": OpSpec(
            description=(
                "Mermaid/drawio 源码文件 → 可编辑 PPTX（16:9 整页）。格式按扩展名+内容"
        ...
        "install_skill": OpSpec(
            description=(
                "把 diagram-design + offipy-diagram skill 安装到宿主 agent 技能目录"
                "（默认 ~/.claude/skills/，--target_dir 指定）。幂等：目标已存在则跳过"
                "（不覆盖用户编辑）；--force 删除并重建目标目录。"
            ),
    ...
    "feedback": {
        ...
        "append": OpSpec(
            description=(
                ...
                "供 feedback train 学习。features 为扁平特征快照（encode_features "
                "产出），CLI 传 JSON 字符串。"
            ),
